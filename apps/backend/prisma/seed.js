...
async function main() {
    ...
    const adminPassword = await bcryptjs_1.default.hash('Admin@2024!', 12);
    const agentPassword = await bcryptjs_1.default.hash('Agent@2024!', 12);
    // Admin
    ...
    await prisma.slaConfig.createMany({
        data: [
            { priority: client_1.TicketPriority.URGENT, firstResponseTime: 60, resolutionTime: 240 }, // 1h / 4h
            { priority: client_1.TicketPriority.HIGH, firstResponseTime: 240, resolutionTime: 1440 }, // 4h / 24h
            { priority: client_1.TicketPriority.MEDIUM, firstResponseTime: 480, resolutionTime: 2880 }, // 8h / 48h
            { priority: client_1.TicketPriority.LOW, firstResponseTime: 1440, resolutionTime: 4320 }, // 24h / 72h
        ],
    });
    ...
    const ticket1 = await prisma.ticket.create({
        data: {
            ...
            issueType: client_1.IssueType.TECHNICAL,
    ...
    const ticket2 = await prisma.ticket.create({
        data: {
            ...
            issueType: client_1.IssueType.DELIVERY,
    ...
    const ticket3 = await prisma.ticket.create({
        data: {
            ...
            issueType: client_1.IssueType.BILLING,
    ...
    await prisma.ticketHistory.create({
        data: {
            ...
            action: client_1.TicketAction.CREATED,
            metadata: { source: 'web' },
        },
    });
    await prisma.ticketHistory.create({
        data: {
            ticketId: ticket1.id,
            actorId: admin.id,
            action: client_1.TicketAction.ASSIGNED,
            field: 'assignedToId',
            oldValue: null,
            newValue: agent1.id,
        },
    ...
    await prisma.ticketHistory.create({
        data: {
            ticketId: ticket2.id,
            actorId: agent2.id,
            action: client_1.TicketAction.STATUS_CHANGED,
            field: 'status',
            oldValue: client_1.TicketStatus.OPEN,
            newValue: client_1.TicketStatus.IN_PROGRESS,
        },
    });
    // History for ticket3
    await prisma.ticketHistory.create({
        data: {
            ...
            action: client_1.TicketAction.STATUS_CHANGED,
            field: 'status',
            oldValue: client_1.TicketStatus.IN_PROGRESS,
            newValue: client_1.TicketStatus.RESOLVED,
        },
    });
    ...
    await prisma.notification.create({
        data: {
            userId: agent1.id,
            type: client_1.NotificationType.TICKET_UPDATE,
            ticketId: ticket1.id,
            payload: {
